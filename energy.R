...
energy_req <- function(a, x, RR, eta, pi){
...
coag <- data.frame('name' = 'coagulation', 'req' = average(0.4, 0.7))
uv <- data.frame('name' = 'uv oxidation', 'req' = average(0.01, 0.05))
o3 <- data.frame('name' = 'ozonation', 'req' = average(0.03, 0.1))
uf <- data.frame('name' = 'ultrafiltration', 'req' = average(0.07, 0.1, 0.2))
mf <- data.frame('name' = 'microfiltration', 'req' = 0.18)
gac <- data.frame('name' = 'granular activated carbon', 'req' = 0.37)
gmf <- data.frame('name' = 'gmf', 'req' = average(0.16, 0.32))
recharge <- data.frame('name' = 'groundwater recharge', 'req' = 0.48)
desal <- data.frame('name' = 'saltwater desalination', 'req' = average(3.5, 4.5))

# combine each of the unit processes into a consolidated data frame
energy_reqs <- rbind(coag, uv, o3, uf, mf, gac, gmf, recharge, desal, energy_reqs)
